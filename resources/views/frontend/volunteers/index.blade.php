@extends('layouts.frontend')
@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            @can('volunteer_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.volunteers.create') }}">
                            {{ trans('global.add') }} {{ trans('cruds.volunteer.title_singular') }}
                        </a>
                    </div>
                </div>
            @endcan
            <div class="card">
                <div class="card-header">
                    {{ trans('cruds.volunteer.title_singular') }} {{ trans('global.list') }}
                </div>

                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-Volunteer">
                            <thead>
                                <tr>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.id') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.first_name') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.last_name') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.phone') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.email') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.gender') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.age') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.experience') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.fields') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.period_time') }}
                                    </th>
                                    <th>
                                        {{ trans('cruds.volunteer.fields.note') }}
                                    </th>
                                    <th>
                                        &nbsp;
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($volunteers as $key => $volunteer)
                                    <tr data-entry-id="{{ $volunteer->id }}">
                                        <td>
                                            {{ $volunteer->id ?? '' }}
                                        </td>
                                        <td>
                                            {{ $volunteer->first_name ?? '' }}
                                        </td>
                                        <td>
                                            {{ $volunteer->last_name ?? '' }}
                                        </td>
                                        <td>
                                            {{ $volunteer->phone ?? '' }}
                                        </td>
                                        <td>
                                            {{ $volunteer->email ?? '' }}
                                        </td>
                                        <td>
                                            {{ App\Models\Volunteer::GENDER_SELECT[$volunteer->gender] ?? '' }}
                                        </td>
                                        <td>
                                            {{ App\Models\Volunteer::AGE_SELECT[$volunteer->age] ?? '' }}
                                        </td>
                                        <td>
                                            {{ $volunteer->experience ?? '' }}
                                        </td>
                                        <td>
                                            {{ App\Models\Volunteer::FIELDS_RADIO[$volunteer->fields] ?? '' }}
                                        </td>
                                        <td>
                                            {{ App\Models\Volunteer::PERIOD_TIME_SELECT[$volunteer->period_time] ?? '' }}
                                        </td>
                                        <td>
                                            {{ $volunteer->note ?? '' }}
                                        </td>
                                        <td>
                                            @can('volunteer_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.volunteers.show', $volunteer->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('volunteer_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.volunteers.edit', $volunteer->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('volunteer_delete')
                                                <form action="{{ route('frontend.volunteers.destroy', $volunteer->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
                                                    <input type="hidden" name="_method" value="DELETE">
                                                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                    <input type="submit" class="btn btn-xs btn-danger" value="{{ trans('global.delete') }}">
                                                </form>
                                            @endcan

                                        </td>

                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>

        </div>
    </div>
</div>
@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('volunteer_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}'
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('frontend.volunteers.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).nodes(), function (entry) {
          return $(entry).data('entry-id')
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  $.extend(true, $.fn.dataTable.defaults, {
    orderCellsTop: true,
    order: [[ 1, 'desc' ]],
    pageLength: 25,
  });
  let table = $('.datatable-Volunteer:not(.ajaxTable)').DataTable({ buttons: dtButtons })
  $('a[data-toggle="tab"]').on('shown.bs.tab click', function(e){
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
  });
  
})

</script>
@endsection