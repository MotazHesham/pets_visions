        ...
        <div class="col-md-12">
            @can('clinic_review_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.clinic-reviews.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-ClinicReview">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $clinicReview->id }}">
                                        ...
                                        <td>
                                            @can('clinic_review_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.clinic-reviews.show', $clinicReview->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('clinic_review_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.clinic-reviews.edit', $clinicReview->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('clinic_review_delete')
                                                <form action="{{ route('frontend.clinic-reviews.destroy', $clinicReview->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('clinic_review_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.clinic-reviews.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-ClinicReview:not(.ajaxTable)').DataTable({ buttons: dtButtons })
