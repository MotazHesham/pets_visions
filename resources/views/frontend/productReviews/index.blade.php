        ...
        <div class="col-md-12">
            @can('product_review_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.product-reviews.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $productReview->id }}">
                                        ...
                                        <td>
                                            @can('product_review_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.product-reviews.show', $productReview->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('product_review_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.product-reviews.edit', $productReview->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('product_review_delete')
                                                <form action="{{ route('frontend.product-reviews.destroy', $productReview->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('product_review_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.product-reviews.massDestroy') }}",
