        ...
        <div class="col-md-12">
            @can('adoption_pet_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.adoption-pets.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-AdoptionPet">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $adoptionPet->id }}">
                                        ...
                                        <td>
                                            @can('adoption_pet_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.adoption-pets.show', $adoptionPet->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('adoption_pet_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.adoption-pets.edit', $adoptionPet->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('adoption_pet_delete')
                                                <form action="{{ route('frontend.adoption-pets.destroy', $adoptionPet->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('adoption_pet_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.adoption-pets.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-AdoptionPet:not(.ajaxTable)').DataTable({ buttons: dtButtons })
