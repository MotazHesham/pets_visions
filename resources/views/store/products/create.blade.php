    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('store.products.store') }}" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="store_id" value="{{ currentStore()->id }}" id="">
                <div class="row"> 
                    ...
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control {{ $errors->has('video_provider') ? 'is-invalid' : '' }}"
                            ...
                            @foreach (App\Models\Product::VIDEO_PROVIDER_SELECT as $key => $label)
                                <option value="{{ $key }}"
                                    {{ old('video_provider', '') === (string) $key ? 'selected' : '' }}>
                                    {{ $label }}</option>
                            @endforeach
                        </select>
...
@section('scripts')
                            ...
                            return loader.file
                                .then(function(file) {
                                    return new Promise(function(resolve, reject) {
                                        ...
                                        xhr.open('POST',
                                            '{{ route('store.products.storeCKEditorImages') }}',
                                            true);
    ...
    <script>
        Dropzone.options.mainPhotoDropzone = {
            url: '{{ route('store.products.storeMedia') }}',
            maxFilesize: 4, // MB
