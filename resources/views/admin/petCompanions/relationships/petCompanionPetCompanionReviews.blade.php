...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-petCompanionPetCompanionReviews">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $petCompanionReview->id }}">
                            ...
                            <td>
                                @can('pet_companion_review_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.pet-companion-reviews.show', $petCompanionReview->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('pet_companion_review_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.pet-companion-reviews.edit', $petCompanionReview->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('pet_companion_review_delete')
                                    <form action="{{ route('admin.pet-companion-reviews.destroy', $petCompanionReview->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-petCompanionPetCompanionReviews:not(.ajaxTable)').DataTable({ buttons: dtButtons })
