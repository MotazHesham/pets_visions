...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-hostingServicesHostings">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($hostings as $key => $hosting)
                        <tr data-entry-id="{{ $hosting->id }}">
                            ...
                            <td>
                                @foreach($hosting->hosting_services as $key => $item)
                                    <span class="badge badge-info">{{ $item->name }}</span>
                                @endforeach
                            </td>
                            <td>
                                @can('hosting_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.hostings.show', $hosting->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('hosting_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.hostings.edit', $hosting->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('hosting_delete')
                                    <form action="{{ route('admin.hostings.destroy', $hosting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-hostingServicesHostings:not(.ajaxTable)').DataTable({ buttons: dtButtons })
